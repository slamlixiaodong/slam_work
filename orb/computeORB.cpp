...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
    ...
    for (auto &kp : keypoints) {
        ...
        if(kp.pt.x<half_patch_size || kp.pt.y<half_patch_size || kp.pt.x+8>image.size().width || kp.pt.y+8>image.size().height)
        //if(kp.pt.x<half_patch_size || kp.pt.y<half_patch_size || kp.pt.x+8>image.rows || kp.pt.y+8>image.cols)
        {
            kp.angle=0;
            continue;
        }
        ...
        for (int v=-8; v<8; ++v) {
            for (int u=-8; u<8; ++u) {
                // at函数：参数1行号（y or v），参数2列号（x or u）
                m_10 += u * image.at<uchar>(kp.pt.y+v, kp.pt.x+u);
                m_01 += v * image.at<uchar>(kp.pt.y+v, kp.pt.x+u);
            }
        }
        kp.angle = std::atan2((float)m_01, (float)m_10) * 180 / pi;
        // END YOUR CODE HERE
    }
    return;
}
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            auto angle=kp.angle/180*pi;
            p_1=cos(angle)*ORB_pattern[4*i]-sin(angle)*ORB_pattern[4*i+1];
            p_2=sin(angle)*ORB_pattern[4*i]+cos(angle)*ORB_pattern[4*i+1];
            q_1=cos(angle)*ORB_pattern[4*i+2]-sin(angle)*ORB_pattern[4*i+3];
            q_2=sin(angle)*ORB_pattern[4*i+2]+cos(angle)*ORB_pattern[4*i+3];
            if(kp.pt.x+p_1<0 || kp.pt.y+q_1<0 || kp.pt.x+p_2<0 || kp.pt.y+q_2<0 ||
               kp.pt.x+p_1>=image.size().width || kp.pt.y+q_1>=image.size().height ||
               kp.pt.x+p_2>=image.size().width || kp.pt.y+q_2>=image.size().height)
            {
                d.clear();
                break;
            }
            //if(image.at<uchar>(kp.pt.y+cvRound(p_2),kp.pt.x+cvRound(p_1))>image.at<uchar>(kp.pt.y+cvRound(q_2),kp.pt.x+cvRound(q_1))) d[i]=0;
            d[i] = (image.at<uchar>(kp.pt.y+cvRound(q_2), kp.pt.x+cvRound(q_1))
                    > image.at<uchar>(kp.pt.y+cvRound(p_2), kp.pt.x+cvRound(p_1))) ? 0 : 1;
            // END YOUR CODE HERE
        }
        desc.push_back(d);
    }
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for (size_t d1_id=0; d1_id<desc1.size(); ++d1_id) {
        if (desc1[d1_id].empty())
            continue;

        cv::DMatch match(d1_id, 0, 256);

        for (size_t d2_id=0; d2_id<desc2.size(); ++d2_id) {
            if (desc2[d2_id].empty())
                continue;
            // 计算d1 d2的距离
            int distance = 0;
            for (int i=0; i<256; ++i) {
                if (desc1[d1_id][i] != desc2[d2_id][i])
                    ++distance;
            }
            if (distance < match.distance) {
                match.trainIdx = d2_id;
